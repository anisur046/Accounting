...
function Reports() {
  ...
  const calculateCoopReports = () => {
    ...
    const tbLines = coopRecords.map(r => {
  ...
  const handleExportCoopExcel = () => {
    ...
    const npaProvisionVal = coopRecords.filter(r => r.type === 'Provisions' && r.head.toLowerCase().includes('npa')).reduce((acc, r) => acc + (r.openingBalance + r.totalCredit - r.totalDebit), 0);
